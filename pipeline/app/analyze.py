...
`config.MODEL` picks the analyzer in `get_analyzer()`; adding another model is
a one-line change (add a branch, nothing else). Two analyzers exist today:

- `HonestMockAnalyzer` (input_kind="palette") — GPU-free, ports the app's real
  colour reasoning from lovanya/lib/ai/mock.ts. Perceives from a colour
  palette. Runs anywhere; use it on machines without the GPU.
- `QwenAnalyzer` (input_kind="image", qwen_analyzer.py) — Qwen2.5-VL judging
  the actual photo against the styling framework. Needs the GPU box.
...
def nearest_occasion(text: str) -> str:
    """Map free-text occasion (bench/CLI input) onto the app's Occasion values."""
    t = (text or "").lower()
    for key, words in (
        ("work", ("work", "office", "business", "interview")),
        ("class", ("class", "school", "campus", "uni")),
        ("date", ("date",)),
        ("event", ("event", "formal", "party", "wedding", "gala", "dinner")),
        ("errands", ("errand", "grocery", "shopping")),
    ):
...
def get_analyzer():
    """Swappable model boundary: config.MODEL picks the analyzer (cached).

    Adding another model is a one-line change: add a branch here.
    """
    global _analyzer
    ...
    if model == "honest-mock":
        _analyzer = HonestMockAnalyzer(load_framework())
    elif model.startswith("qwen"):
        # Heavy import (torch + transformers) deferred until the model is chosen.
        from .qwen_analyzer import QwenAnalyzer

        _analyzer = QwenAnalyzer()
    else:
        raise ValueError(
            f"Unknown LOVANYA_MODEL={model!r} (expected 'honest-mock' or 'qwen...')."
        )
    return _analyzer


def analyze(image_path: str, occasion: str) -> dict:
    """One-shot bench/CLI entry: photo file + occasion -> perception + judgment.

    Works with either analyzer: Qwen perceives the photo itself; the mock
    perceives a palette extracted from the photo.
    """
    from .color import extract_palette
    from .logging_store import log_judgment
    from .segment import segment_path
    ...
    if getattr(analyzer, "input_kind", "palette") == "image":
        ...
        ctx = AnalyzeRequest(palette=palette, occasion=nearest_occasion(occasion))  # type: ignore[arg-type]
        perception = analyzer.perceive(palette).model_dump()
        judgment = analyzer.judge(Perception(**perception), palette, ctx).model_dump()
    ...
    log_judgment("analyze-bench", config.MODEL,
                 {"image_path": image_path, "occasion": occasion,
                  "judgment_id": record["judgment_id"]},
                 {"perception": perception, "judgment": judgment})
    return record
